...
class CIRR(torch.utils.data.Dataset):
    def __init__(self, path, transform=None):
        super().__init__()
        self.path = path
        self.cirr_path = "/amax/home/chendian/dataset/cirr_dataset"
        self.dataset_path = "/amax/home/chendian/dataset/cirr_dataset/cirr/"
        self.caption_dir = self.dataset_path + 'captions/'
        self.split_dir = self.dataset_path + 'image_splits/'
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        out = {}
        out['source_img_data'] = self.get_img(pathlib.Path(self.train_image_path[reference_name]), 0)
        out['target_img_data'] = self.get_img(pathlib.Path(self.train_image_path[target_name]), 0)
        out['mod'] = {'str':mod_str}
        ...
        out['visual_query'] = self.get_written_img(os.path.join(self.cirr_path, pathlib.Path(self.train_image_path[reference_name])), key_word, 0)
    ...
    def get_img(self, img_path, stage=0):
        img_path = os.path.join(self.cirr_path, img_path)
        with open(img_path, 'rb') as f:
            img = PIL.Image.open(f)
            img = img.convert('RGB')
    ...
    def get_val_queries(self):
        ...
        for idx in range(len(val_data)):
            ...
            out['group_members'] = subset_names
            
            key_word = self.key_words_val[reference_name + '+' + target_name][-1]
            out['textual_query'] = self.val_captions[reference_name] + ', but' + mod_str
            out['visual_query'] = self.get_written_img(os.path.join(self.cirr_path, pathlib.Path(val_image_path[reference_name])), key_word, 1)
        ...
        for i in range(len(val_image_name)):
            ...
            out['image_name'] = name
            test_targets.append(out)
        # 相当于 test_queries 为 relative / test_targets 为 classic
        # test_targets
        ## images => target_img_data 
        ## names => image_name
        return test_queries, test_targets
    ...
    def get_test_queries(self):
        ...
        for i in range(len(test_data)):
            ...
            out['visual_query'] = self.get_written_img(os.path.join(self.cirr_path, pathlib.Path(test_image_path[caption['reference']])), key_word, 1)
            queries.append(out)
