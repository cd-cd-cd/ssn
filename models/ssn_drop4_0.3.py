...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.text_token_selection_mask = nn.Sequential(nn.Linear(self.width, 1), nn.Sigmoid())
        self.image_token_selection_mask = nn.Sequential(nn.Linear(self.width, 1), nn.Sigmoid())
        
        # self.text_token_selection_mask = nn.Sequential(nn.Linear(self.width * 2, self.width),  nn.ReLU(), nn.Dropout(0.5), nn.Linear(self.width, 1), nn.Sigmoid())
        # self.image_token_selection_mask = nn.Sequential(nn.Linear(self.width * 2, self.width), nn.ReLU(), nn.Dropout(0.5), nn.Linear(self.width, 1), nn.Sigmoid())
        
        # self.text_token_selection_mask = nn.Sequential(nn.Linear(self.width * 2, self.width),  nn.ReLU(), nn.Dropout(0.5), nn.Linear(self.width, self.clip_feature_dim))
        # self.image_token_selection_mask = nn.Sequential(nn.Linear(self.width * 2, self.width), nn.ReLU(), nn.Dropout(0.5), nn.Linear(self.width, self.clip_feature_dim))
        ...
        self.q_weight_layer = Router(2, self.projection_dim, self.hidden_dim)
        self.dynamic_vector = nn.Sequential(nn.Linear(self.projection_dim * 2, self.hidden_dim), nn.ReLU(), nn.Dropout(0.5),
                                            nn.Linear(self.hidden_dim, self.clip_feature_dim), nn.Sigmoid())
        ...
        self.mu = args.mu
        self.weighted_layer = nn.Linear(self.clip_feature_dim, 3)
        self.crossAttention = CrossAttention(self.clip_feature_dim, args.n_layers, args.n_heads, None)
        self.feed_forward_layer = FeedForward(self.projection_dim, int(args.embed_size * 4), drop=0.2)
        self.norm2 = nn.LayerNorm(self.projection_dim)
    ...
    def combine_features(self, reference_embeds, text_embeds, reference_features, text_features, text_mask= None) -> torch.tensor:
        ...
        if text_features is not None:
            ...
            output = ((dynamic_scalar + self.mu * dynamic_vector) * text_embeds + \
                                            (1 - (dynamic_scalar + self.mu * dynamic_vector)) * reference_embeds)
        else:
            output = 0.01 * cls_ref_embeds + reference_embeds
        return F.normalize(output, dim=-1)
    ...
    #         cat_feats = self.output_layer(combined_features)
    #         weighted = self.weighted_layer(cat_feats)
            
    #         output = (
    #         weighted[:, 0:1] * text_embeds
    #         + weighted[:, 1:2] * cat_feats
    #         + weighted[:, 2:3] * reference_embeds
    #        )
    #     else:
    #         output = 0.01 * cls_ref_embeds + reference_embeds
    #     return F.normalize(output, dim=-1)
    ...
    def encode_features(self, image_features: torch.tensor, text_outputs, text_mask) -> torch.tensor:
        if text_outputs is not None:
            ...
            cross_feat = self.crossAttention(local_x, global_img.expand(B, N, C))
            out = cross_feat + self.dropout4(self.feed_forward_layer(self.norm2(cross_feat)))
            # expand_features = torch.cat([local_x, global_img.expand(B, N, C)], dim=-1)
            selection_mask = self.text_token_selection_mask(out)
            # selection_mask = self.self_attn(selection_mask.unsqueeze(1)).squeeze(1)
            ...
            out = cross_feat + self.dropout4(self.feed_forward_layer(self.norm2(cross_feat)))
            out = cross_feat
            # expand_features = torch.cat([cross_feat, global_x.expand(B, N, C)], dim=-1)
            # expand_features = torch.cat([local_x, global_x.expand(B, N, C)], dim=-1)
            selection_mask = self.image_token_selection_mask(out)
            # selection_mask = self.self_attn(selection_mask.unsqueeze(1)).squeeze(1)
