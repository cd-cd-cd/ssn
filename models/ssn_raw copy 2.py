...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.output_layer = nn.Linear(self.projection_dim * 2, self.clip_feature_dim)
        ...
        # edit
        self.transformer = EfficientAdditiveAttnetion(in_dims=self.projection_dim, token_dim=512)
        self.transformer2 = ExternalAttention(d_model=self.projection_dim)
    ...
    def combine_features(self, reference_embeds, text_embeds, reference_features, text_features, text_mask= None) -> torch.tensor:
        ...
        if text_features is not None:
            ...
            # cls_ref_embeds = self.transformer(cls_ref_embeds).squeeze(1)
            # cls_text_embeds = self.transformer(cls_text_embeds).squeeze(1)
    ...
    def encode_features(self, image_features: torch.tensor, text_outputs, text_mask) -> torch.tensor:
        if text_outputs is not None:
            ...
            pos_fused_features = self.transformer2(pos_fused_features)
            ...
            neg_fused_features = self.transformer2(neg_fused_features)
