...
class CAFM(nn.Module):  # Cross Attention Fusion Module
    def __init__(self):
        ...
        self.avg1 = nn.Conv2d(1024, 64, 1, stride=1, padding=0)
        self.avg2 = nn.Conv2d(1024, 64, 1, stride=1, padding=0)
        self.max1 = nn.Conv2d(1024, 64, 1, stride=1, padding=0)
        self.max2 = nn.Conv2d(1024, 64, 1, stride=1, padding=0)

        self.avg11 = nn.Conv2d(64, 1024, 1, stride=1, padding=0)
        self.avg22 = nn.Conv2d(64, 1024, 1, stride=1, padding=0)
        self.max11 = nn.Conv2d(64, 1024, 1, stride=1, padding=0)
        self.max22 = nn.Conv2d(64, 1024, 1, stride=1, padding=0)
...
class LinAngularXCA_CA(nn.Module):
    def __init__(self):
        ...
        self.la = LinAngularAttention(in_channels=1024)
        self.xa = XCA(dim=1024)
        self.cafm = CAFM()
    ...
    def forward(self, x):
        ...
        # result = result.permute(1, 2, 0) # 首先交换维度，变为 [32, 784, 1024]
