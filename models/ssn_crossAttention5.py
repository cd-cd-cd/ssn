...
from models.MUSE import MUSEAttention
from models.EMSA import EMSA
from models.MobileViTv2Attention import MobileViTv2Attention
from models.SpectFormer import SpectralGatingNetwork
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.text_token_selection_mask = nn.Sequential(nn.Linear(self.width * 3, self.width), nn.Sigmoid(), nn.Linear(self.width, 1), nn.Sigmoid())
        self.image_token_selection_mask = nn.Sequential(nn.Linear(self.width * 3, self.width), nn.Sigmoid(), nn.Linear(self.width, 1), nn.Sigmoid())
        ...
        self.muse = MUSEAttention(d_model=self.width, d_k=32, d_v=32, h=16)
        self.mobile = MobileViTv2Attention(d_model=self.width)
    ...
    def encode_features(self, image_features: torch.tensor, text_outputs, text_mask) -> torch.tensor:
        if text_outputs is not None:
            ...
            mobile_fedat = self.mobile(co_features)
            ...
            pos_fused_features = self.crossAttention(pos_fused_features, mobile_fedat)
            ...
            neg_fused_features = self.crossAttention(neg_fused_features, mobile_fedat)
