...
class Model(nn.Module):
    ...
    def combine_features(self, reference_embeds, text_embeds, reference_features, text_features, text_mask= None) -> torch.tensor:
        ...
        if text_features is not None:
            ...
            cross_text = self.crossAttention(cls_text_embeds, text_embeds)
            cross_text = torch.sum(cross_text, dim=1)
            cross_feat = self.crossAttention(cls_ref_embeds, reference_embeds)
            cross_feat = torch.sum(cross_feat, dim=1)
            raw_combined_features = torch.cat((cross_text, cross_feat), -1)
            # raw_combined_features = torch.cat((cls_ref_embeds, cls_text_embeds), -1)
