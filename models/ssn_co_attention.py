...
from models.co_attention import Co_attention_block, CrossAttention
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.text_token_selection_mask = nn.Sequential(nn.Linear(self.width * 4, 1), nn.Sigmoid())
        self.image_token_selection_mask = nn.Sequential(nn.Linear(self.width * 4, 1), nn.Sigmoid())
        ...
        # edit
        self.fusion_layer2 = nn.TransformerEncoder(encoder_layer, num_layers=1)
        self.co_connetion_transformer_model_block = Co_attention_block(hidden_size=self.projection_dim, num_attention_heads=args.n_heads, dropout_rate=0.1)
    ...
    def encode_features(self, image_features: torch.tensor, text_outputs, text_mask) -> torch.tensor:
        if text_outputs is not None:
            ...
            image_layer_output, text_layer_output, co_attention_probs = self.co_connetion_transformer_model_block(global_img, local_x)
            expand_features1 = self.agg_video_feat(text_layer_output)
            expand_features2 = self.agg_video_feat(image_layer_output)
            expand_features = torch.cat([expand_features1, expand_features2, global_img, local_x], dim=-1)
            # expand_features = torch.cat([image_layer_output, text_layer_output], dim=-1)
            ...
            image_layer_output, text_layer_output, co_attention_probs = self.co_connetion_transformer_model_block(global_x, local_x)
            expand_features1 = self.agg_video_feat(text_layer_output)
            expand_features2 = self.agg_video_feat(image_layer_output)
            expand_features = torch.cat([expand_features1, expand_features2, global_x, local_x], dim=-1)
            # expand_features = torch.cat([image_layer_output, text_layer_output], dim=-1)
    ...
    def agg_video_feat(self, image_layer_output):
        visual_output = image_layer_output.contiguous()

        visual_output = visual_output.permute(1, 0, 2)  # NLD -> LND
        visual_output = self.fusion_layer(visual_output)
